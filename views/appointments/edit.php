...
<h2 style="margin-top:8px;margin-bottom:4px;">Editar agendamento</h2>
...
<div class="card" style="margin-top:8px;max-width:900px;">
  ...
  <form method="post" style="display:flex;flex-direction:column;gap:16px;">
        ...
        >
          <?php foreach (($appointmentTypes ?? []) as $t): ?>
            <option value="<?= htmlspecialchars($t->code) ?>"
              ...
              <?= htmlspecialchars($t->name) ?><?= !$t->active ? ' (inativo)' : '' ?>
            </option>
          <?php endforeach; ?>
        </select>
      </div>
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;margin-top:8px;">
      <a href="<?= BASE_URL ?>/?controller=appointment&action=delete&id=<?= (int)$appointment->id ?>"
         class="btn"
         style="padding:8px 14px;border-radius:999px;border:1px solid var(--danger);
